...
- [ ] GIVEN 后端启动 WHEN `GET /api/system/info` THEN 返回 release、flywayVersion、gitCommit
- [ ] GIVEN `npm run build` WHEN 查看 dist THEN 页脚可见前端版本号
- [ ] GIVEN `./scripts/release/build.sh 0.2.0` WHEN 解压产物 THEN manifest 含 FE/BE 版本与 SHA256
- [ ] GIVEN upgrade-guide WHEN 运维按文档操作 THEN 可在单机完成 Nginx+JAR 部署与 smoke
- [ ] GIVEN FE 版本高于 manifest 允许范围 WHEN 打开首页 THEN 显示版本不匹配提示（若实现 R2 横幅）

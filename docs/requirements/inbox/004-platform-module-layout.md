...
- [ ] GIVEN 任一业务路由 WHEN 进入页面 THEN 显示顶栏 + 模块侧栏 + 面包屑 + page-head 的标准骨架
- [ ] GIVEN 侧栏配置数组 WHEN 新增一项 THEN 无需改布局组件即出现新菜单
- [ ] GIVEN 某模块的子视图 WHEN 通过 URL 直接访问 THEN 能定位到该子视图（深链可用）
- [ ] GIVEN 首页 7 功能卡 / 顶栏项 WHEN 点击 THEN 跳转到对应模块占位路由
- [ ] GIVEN `sysu-cls` / `sysu-org` mock WHEN 在组件中 import THEN 以 TS 类型化模块提供数据
- [ ] GIVEN `npm run build` 与 `npm run test` THEN 通过
...
- 🟡 中等技术 change：Plan Mode；计划落 `docs/plans/`，必要时 ADR

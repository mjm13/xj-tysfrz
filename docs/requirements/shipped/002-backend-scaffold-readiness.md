...
| 启动类 + 包结构 `com.xj.zbpt` | `ZbptApplication.java` |
...
| 项 | 影响 | 优先级 |
| --- | --- | --- |
| **项目命名仍用「指标平台 / zbpt」** | OpenAPI 标题、pom 描述、README、库名 `xj_zbpt` 与产品不一致 | P1 |
...
2. **元信息对齐**：OpenAPI title/description、pom `description`、backend README 改为身份数据平台表述（包名 `com.xj.zbpt` 可暂保留，避免大面积重命名）
...
- 包名重构 `com.xj.zbpt` → `com.xj.tysfrz`（工作量大，非阻塞联调）

# 关键决策（建议）

| 决策 | 建议 | 理由 |
| --- | --- | --- |
| 数据库名 | 短期保留 `xj_zbpt`，文档注明别名 | 避免迁移成本；后续可 Flyway + 配置切换 |
